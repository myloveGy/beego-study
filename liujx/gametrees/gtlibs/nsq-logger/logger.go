...
const (
	ENV_NSQD         = "NSQD_HOST"
	DEFAULT_PUB_ADDR = "http://172.24.10.13:4151/mpub?topic=LOG&binary=true"
...
func publish(msg LogFormat) {
	...
	if msg.Level >= ANALYSIS{ //分析日志 入NSQ
		// pack message
...
/**
 * 2016年1月22日-下午2:07:38<br>
 * 用于分析的日志
 */
func Analysis(v ...interface{}){
	msg := LogFormat{Level: ANALYSIS, Msg: fmt.Sprint(v...)}
	publish(msg)
}

func Analysisf(format string, v ...interface{}) {
	msg := LogFormat{Level: ANALYSIS, Msg: fmt.Sprintf(format, v...)}
	publish(msg)
}
