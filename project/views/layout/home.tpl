...
<script type="text/javascript">
    var isHave = false, Obj = {errorPlacement:validate},lLayer, l, loginurl = "{:U('Index/login')}";
    // 请求错误回应
    ...
    $(function(){
        // 用户退出
        $('.logout').click(function(){if (isLogin()){layer.confirm('您确定要退出登录吗?', {title:'温馨提醒',btn: ['确定退出', '取消'],icon:3,shift:4,}, function(){l = layer.load();$.get('{:U("Index/logout")}', function(json){layer.close(l);layer.msg(json.msg, {time:1000, end:function(){if (json.status == 1) changUser(null)}});}, 'json');}, function(){layer.msg('您取消了退出登录！', {time:1000});});}});
        // 登录可以操作
        ...
        // 发布文章
        $('.btn-article').click(function(){if (isLogin()){if ($('.article').validate(Obj).form()){l = layer.load();$.ajax({url: '{:U("User/insert")}', data:$('.article').serialize(),type:'post',dataType:'json',success:function(json){layer.close(l);var s = json.status == 1 ? 6 : 5;if (json.status == 1) {$('#myModal').modal('hide');}layer.msg(json.msg, {time:2000, icon:s, end:function(){article([json.data]);}})},error:requestError,})}}return false;})
        // 弹出model
        $('.file-upload').click(function(){$('#myImage').modal();});
        // 关闭modal
        $('#myImage').on('hide.bs.modal', function(e){document.image.reset();});
        // 上传图片
        $('.btn-image').click(function(){if (isLogin()){if ($('.image').validate(Obj).form()){l = layer.load();$.ajax({url: '{:U("User/addImg")}', data:$('.image').serialize(),type:'post',dataType:'json',success:function(json){layer.close(l);var s = json.status == 1 ? 6 : 5;if (json.status == 1) {$('#myImage').modal('hide');}layer.msg(json.msg, {time:2000, icon:s})},error:requestError,})}}return false;})
        // 图片显示
