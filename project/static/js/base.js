...
function createRadio(data,checked,params){
    ...
    if(data!=undefined&&typeof data=="object"){
        for(var i in data){
            var check=checked==i?' checked="checked" ':"";html+='<label class="line-height-1 blue"> <input type="radio" '+params+check+' value="'+i+'"  /> <span class="lbl"> '+data[i]+" </span> </label>　 "}}return html}
function createSelect(data,selected,params){var html="",params=handleParams(params);if(data!=undefined&&typeof data=="object"){html+="<select "+params+">";for(var i in data){var select=i==selected?' selected="selected" ':"";html+='<option value="'+i+'" '+select+" >"+data[i]+"</option>"}html+="</select>"}return html}function createFile(params){var html="";if(params==undefined){params={}}html += createInput("hidden",params);params["name"]="UploadForm[" + params["name"] + "]";params["class"]="input-file uniform_on fileUpload";html+=createInput("file",params);
