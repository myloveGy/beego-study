...
// 推荐信息
function recommendToString(td, data) {
    $(td).html('<span class="label label-' + (data == 1 ? 'success">推荐' : 'warning">不推荐') + '</span>');
}
...
var oOperateDetails = {
    "data": null, "title": "操作", "bSortable": false, "createdCell": function (td, data, rowArr, row, col) {
        $(td).html(createButtons([
            {
                "data": row,
                "title": "查看",
                "className": "btn-success",
                "cClass": "me-table-view-detail",
                "icon": "fa-search-plus",
                "sClass": "blue"
            },
            {
...
var MeTable = (function ($) {
    ...
    function MeTable(options, tableOptions, detailOptions) {
        ...
        this.tableOptions.fnServerData = function (sSource, aoData, fnCallback) {
            ...
            // 添加排序字段信息
            if (aoData[mSort].value != undefined && aoData[mSort].value != "") aoData.push({
                "name": 'params[orderBy]',
                "value": attributes[parseInt(aoData[mSort].value)]
            });
            ...
            $.ajax({
                ...
                if (data.code !== 10000) {
                    return layer.msg('出现错误:' + data.msg, {time: 2000, icon: 5});
                }
                    ...
                    "fnServerData": function (sSource, aoData, fnCallback) {
                        if (self.oDetailParams) {
                            ...
                            $.ajax({
                                ...
                                $.fn.dataTable.defaults['bFilter'] = true;
                                fnCallback(data.data);
                                if (self.oDetailObject) self.oDetailObject.child(function () {
                                    return $('#detailTable').parent().html();
                                }).show()
                            }).fail(ajaxFail);
                        }
    ...
    MeTable.prototype.init = function () {
        ...
        if (this.options.sSearchType == 'middle') {
            ...
            $('.me-search').on('keyup change', function () {
                self.table.draw();
            }); 						// 搜索事件
        ...
        $('.me-table-insert').click(function (evt) {
            evt.preventDefault();
            self.insert();
        });
        $(document).on('click', '.me-table-edit', function (evt) {
            evt.preventDefault();
            self.update($(this).attr('table-data'))
        });
        $(document).on('click', '.me-table-del', function (evt) {
            evt.preventDefault();
            self.delete($(this).attr('table-data'))
        });
        $(document).on('click', '.me-table-view', function (evt) {
            evt.preventDefault();
            self.view($(this).attr('table-data'))
        });
        $('.me-table-delete').click(function (evt) {
            evt.preventDefault();
            self.deleteAll();
        });
        $('.me-table-save').click(function (evt) {
            evt.preventDefault();
            self.save();
        });
        $('.me-table-reload').click(function (evt) {
            evt.preventDefault();
            self.search();
        });
        ...
        if (self.bHandleDetails) {
            ...
            // 新增、查看、编辑、删除
            $('.me-table-insert-detail').click(function (evt) {
                evt.preventDefault();
                self.insert(true);
            });
            $(document).on('click', '.me-table-view-detail', function (evt) {
                evt.preventDefault();
                self.view($(this).attr('table-data'), true)
            });
            $(document).on('click', '.me-table-edit-detail', function (evt) {
                evt.preventDefault();
                self.update($(this).attr('table-data'), true)
            });
            $(document).on('click', '.me-table-del-detail', function (evt) {
                evt.preventDefault();
            ...
            $(self.options.sTable + ' tbody').on('click', self.oDetails.sClickSelect, function () {
                ...
                tr.siblings(tr).each(function () {
                    if (self.table.row($(this)).child.isShown()) self.table.row($(this)).child.hide();
                ...
                if (row.child.isShown()) {
                    ...
                    self.oDetailParams = row.data();
                    self.oDetailObject = row;
                    self.details.draw();
                    tr.addClass('shown');
                }
            });
    ...
    MeTable.prototype.view = function (row, isDetail) {
        ...
        var self = this, data = this.table.data()[row], obj = this.tableOptions.aoColumns, t = self.options.sTitle,
            c = '.data-info-', i = "#data-info";
    ...
    MeTable.prototype.save = function (data) {
        ...
        $.ajax({
            ...
            layer.msg(json.msg, {icon: json.code === 10000 ? 6 : 5})
            // 判断操作成功
            if (json.code === 10000) {
                // 执行之后的数据处理
